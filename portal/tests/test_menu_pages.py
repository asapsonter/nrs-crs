...
def test_create_manual_blocked_for_checker(portal_client: Client) -> None:
    from portal.models import Filing

    resp = portal_client.post(
        "/portal/filings/create/",
        {"name": "Should not exist", "filing_type": "MANUAL", "period_end_date": "2024-12-31"},
    )
    assert resp.status_code == 200  # re-renders with error
    assert not Filing.objects.filter(name="Should not exist").exists()
...
def test_tree_shows_editable_actions_and_records_for_maker(maker_client: Client) -> None:
    ...
    assert ">Edit<" in html and ">Add<" in html and ">Delete<" in html
